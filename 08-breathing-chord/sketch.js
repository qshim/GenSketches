...
function draw() {
  ...
  const baseSize = min(width, height) * sizeSlider.value();
  ...
  // ---------- Layout --------------------------------------------------
  // Constrain the chord to the central third of the screen, horizontally
  // centered. The circles stay close together so the chord reads as a
  // unified, agent-like cluster rather than a wide instrument.
  const cy = height * 0.5;
  const targetWidth = width / 3;
  const spacing = targetWidth / (N - 1);
  const totalW = spacing * (N - 1);
  const startX = (width - totalW) / 2;

  // Drift amplitude grows with loudness — the same wave, bigger sweep.
  const ampPx = min(height * 0.34, baseSize * 4.2)
              * ampFactor * (1 + listen * 1.0);
